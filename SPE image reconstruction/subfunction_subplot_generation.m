function [] = subfunction_subplot_ret_generation(mode,range_ret_dep, img2write1,img2write2, img2write3)
...
if strcmp(mode,'ret');
    ...
    h2 = subplot(1, 2, 2);
    imshow(img2write2);title('Ret Image no colorbar','FontSize', fontSize); 
...
if strcmp(mode,'dep');
    ...
    originalSize2 = get(gca, 'Position');
    %uiwait(msgbox('Click OK to see a colorbar and watch how it changes both axes sizes'));
    colormap(gray(256));caxis(range_ret_dep);
    cbh =colorbar('FontSize', fontSize);
    cbh.Ticks = linspace(range_ret_dep(1), range_ret_dep(2), 6) ; %Create 8 ticks from zero to 1
