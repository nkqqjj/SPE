function out = imoverlay(in, mask, color, alpha)
...
out_red(mask)   = in_uint8(mask).*alpha+color_uint8(1)*(1-alpha);
out_green(mask) = in_uint8(mask).*alpha+color_uint8(2)*(1-alpha);
out_blue(mask)  = in_uint8(mask).*alpha+color_uint8(3)*(1-alpha);
